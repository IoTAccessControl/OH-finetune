...
★ 本脚本做的事：
  ...
  3. 逐条保存原文到 results_test_200_dump.jsonl，并对"未产出规范 collectionAndUse"的样本
...
def main():
    ...
    with open(os.path.join(out_dir, "results_test_200.json"), "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    # 逐条原文 dump（合并 adapter + base）
    dump = []
    for ra, rb in zip(rows_a, rows_b):
        dump.append({"idx": ra["idx"], "adapter": ra, "base": rb})
    with open(os.path.join(out_dir, "results_test_200_dump.jsonl"), "w", encoding="utf-8") as f:
        for d in dump:
            f.write(json.dumps(d, ensure_ascii=False) + "\n")
    print(f">>> 结果已写 results_test_200.json + results_test_200_dump.jsonl")
