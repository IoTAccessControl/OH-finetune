...
用法（在仓库 scripts/ 目录下）：
  CUDA_VISIBLE_DEVICES=0 python evaluate_twostage_test_7b.py \\
...
if "BASE_MODEL" not in os.environ:
    raise SystemExit(
        ...
        "  export BASE_MODEL=/path/to/Qwen2.5-7B-Instruct\n"
        "或运行：BASE_MODEL=/path/to/Qwen2.5-7B-Instruct python evaluate_twostage_test_7b.py --adapter <path>"
    )
...
def main():
    ...
    adapter_default = os.path.join(REPO_ROOT, "output_twostage_7b")
    ap.add_argument("--adapter",
                    default=adapter_default,
                    help="LoRA adapter 目录路径（训练产物）。"
                         "默认: <仓库根>/output_twostage_7b")
